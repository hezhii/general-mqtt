...
class Pinger {
  ...
  private doPing = () => {
    if (this.isReset) {
      ...
      this.timeout = BackgroundTimer.setTimeout(this.doPing, this.keepAliveInterval)
    } else {
  ...
  reset() {
    ...
    if (this.keepAliveInterval > 0) this.timeout = BackgroundTimer.setTimeout(this.doPing, this.keepAliveInterval)
  }
